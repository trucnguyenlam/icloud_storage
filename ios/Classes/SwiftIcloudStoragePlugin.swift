...
public class SwiftIcloudStoragePlugin: NSObject, FlutterPlugin {
  ...
  private func listChildrenOfDirectoryInICloud(at directoryURL: URL, container containerURL: URL) -> [[String: Any?]] {
      var fileMaps: [[String: Any?]] = []

      let fileCoordinator = NSFileCoordinator()
      fileCoordinator.coordinate(readingItemAt: directoryURL, options: [], error: nil) { (directoryURL) in
          do {
  ...
  private func getFile(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    ...
    DebugHelper.log("directoryURL: \(fileURL.path)")
    ...
    fileCoordinator.coordinate(readingItemAt: fileURL, options: [], error: nil) { (newURL) in
        do {
            let attributes = try newURL.resourceValues(forKeys: [.contentModificationDateKey, .creationDateKey, .fileSizeKey])
  ...
  func fastDownload(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    guard let args = call.arguments as? Dictionary<String, Any>,
          let containerId = args["containerId"] as? String,
    ...
    let downloadStreamHandler: StreamHandler? = self.streamHandlers.removeValue(forKey: eventChannelName)

    let sessionConfig = URLSessionConfiguration.default
    let session = URLSession(configuration: sessionConfig, delegate: DownloadDelegate(destinationURL: destinationURL, streamHandler: downloadStreamHandler), delegateQueue: nil)
